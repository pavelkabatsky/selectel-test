...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var rangeStyle = require('./range-style.js');

  var forEachPolyfill = require('./forEachPolyfill.js');
...
},{"./forEachPolyfill.js":310,"./main.js":312,"./range-style.js":313,"@babel/polyfill":1,"whatwg-fetch":309}],312:[function(require,module,exports){
(function (process){
"use strict";
...
module.exports = function () {
  var wrap = document.querySelector('.tariffs-cards');
  var loader = document.querySelector('.loader');
  var range = document.querySelector('.range-selector');
  var configurations = [];
  var checkboxes = document.querySelectorAll('.check__input');
  var checkFilter = []; // Получение данных c сервера

  var getData = function getData() {
    return window.fetch('https://api.jsonbin.io/b/5df3c10a2c714135cda0bf0f/1').then(function (response) {
  ...
  var filters = {
    ...
    filterByRange: function filterByRange(data, rangeVal) {
      return data.filter(function (item) {
        var procQ = item.cpu.cores * item.cpu.count;
        return procQ === rangeVal;
      });
    },
    // метод сортировки checkbox
    filterByCheckbox: function filterByCheckbox(data, checkFilter) {
      var filteredElements = _toConsumableArray(data);

      checkFilter.forEach(function (item) {
        if (item === 'GPU') {
          filteredElements = _toConsumableArray(filteredElements.filter(function (item) {
            return item.gpu;
          }));
        } else if (item === 'RAID') {
          filteredElements = _toConsumableArray(filteredElements.filter(function (item) {
  ...
  var process = function process(data) {
    configurations = _toConsumableArray(data);
    filterConfigs(configurations);
    loader.style.display = 'none';
  }; // Пропуск полученных данных через фильтры 


  var filterConfigs = function filterConfigs(configurations) {
    // Первичная сортировка range 
    var newArr = filters.filterByRange(configurations, filters.rangeValue);
    render(newArr); // Cортировка при перетаскивании range

    range.addEventListener('change', function () {
      filters.rangeValue = +this.value;

      if (checkFilter.length) {
        var sortByBoxArr = filters.filterByCheckbox(configurations, checkFilter);

        var _sortByRangeArr = filters.filterByRange(sortByBoxArr, filters.rangeValue);

        render(_sortByRangeArr);
        return;
      }

      var sortByRangeArr = filters.filterByRange(configurations, filters.rangeValue);
    ...
    checkboxes.forEach(function (item) {
      item.addEventListener('change', function (e) {
        if (item.checked) {
          checkFilter = [].concat(_toConsumableArray(checkFilter), [e.target.value]);
          var sortByRangeArr = filters.filterByRange(configurations, filters.rangeValue);
          var sortByBoxArr = filters.filterByCheckbox(sortByRangeArr, checkFilter);
          render(sortByBoxArr);
        } else {
          checkFilter = _toConsumableArray(checkFilter.filter(function (item) {
            return item != e.target.value;
          }));

          var _sortByRangeArr2 = filters.filterByRange(configurations, filters.rangeValue);

          if (checkFilter.length) {
            var _sortByBoxArr = filters.filterByCheckbox(_sortByRangeArr2, checkFilter);

            render(_sortByBoxArr);
            return;
          }

          render(_sortByRangeArr2);
        }
      });
    });
  ...
  var render = function render(arr) {
    ...
    arr.forEach(function (item) {
      ...
      cardItem.innerHTML = "<ul>\n                    <li class=\"tariffs-cards__item-config\"><span>".concat(item.name, "</span> ").concat(item.cpu.count >= 2 ? item.cpu.count + ' x ' : '', " ").concat(item.cpu.name, ", ").concat(item.cpu.count * item.cpu.cores, " \u044F\u0434\u0435\u0440</li>\n                    <li class=\"tariffs-cards__item-ram\">").concat(item.ram, "</li>\n                    <li class=\"tariffs-cards__item-hard\">").concat(item.disk.count >= 2 ? item.disk.count + ' x ' : '', " ").concat(item.disk.count, " \u0413\u0411 ").concat(item.disk.type, "</li>\n                </ul>\n                <div class=\"tariffs-cards__item-bottom\">\n                    <div class=\"tariffs-cards__item-price\">").concat(item.price / 100, " \u20BD/\u043C\u0435\u0441\u044F\u0446</div>\n                    <a class=\"order-btn\" href=\"https://selectel.ru/\" target=\"_blanc\"> \u0417\u0430\u043A\u0430\u0437\u0430\u0442\u044C</a>\n                </div>");
...
},{"_process":308}],313:[function(require,module,exports){
"use strict";
...
module.exports = function () {
  ...
  var x = (startValue / (maxVal - minVal) - 0.2) * 100;
  var color = 'linear-gradient(90deg, rgb(47,147,254)' + x + '% , rgb(229, 229, 229)' + x + '%)';
